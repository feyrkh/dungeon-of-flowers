...
func setup(_track_bonuses, _block_height):
	track_bonuses = _track_bonuses
	block_height = _block_height
	top_bonus_label_y = 0
	update_label_tracks()

func get_total_bonuses():
	var bonuses = {}
	for bonus_label in bonus_labels:
		if bonus_label.earned:
			bonuses[bonus_label.bonus_type] = bonuses.get(bonus_label.bonus_type, 0) + bonus_label.bonus_amount
