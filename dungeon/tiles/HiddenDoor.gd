...
func on_map_orientation(orientation_vector, orientation_name):
	match orientation_name:
		"south": can_open_from_map_coords = map_position + Vector2.UP
		"north": can_open_from_map_coords = map_position + Vector2.DOWN
		"west": can_open_from_map_coords = map_position + Vector2.RIGHT
		"east": can_open_from_map_coords = map_position + Vector2.LEFT

func is_interactable():
	return !animating and player_standing_on_right_side()

func player_standing_on_right_side():
	return Util.map_coords(GameData.player.transform.origin) == can_open_from_map_coords
